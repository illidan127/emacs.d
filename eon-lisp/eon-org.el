...
(defvar eon-org-agenda--restriction nil
  "当前 `eon-org-agenda' 的范围限制，作为 `org-agenda' 第三参数。")

(defun eon-org-agenda--restriction-label ()
  (pcase eon-org-agenda--restriction
    ('buffer "当前 buffer")
    ('subtree "当前 subtree")
    ('region "当前 region")
    (_ "无限制")))

(defun eon-org-agenda--restriction-refresh ()
  (message "Agenda 范围: %s" (eon-org-agenda--restriction-label))
...
(eon-org-agenda--def-cmd "s" "Agenda：关键词搜索")
(eon-org-agenda--def-cmd "S" "Agenda：关键词搜索（仅 TODO）")
(eon-org-agenda--def-cmd "/" "Agenda：multi-occur" "occur")
(eon-org-agenda--def-cmd "?" "Agenda：FLAGGED 条目" "flagged")
(eon-org-agenda--def-cmd "#" "Agenda：stuck 项目" "stuck")
(eon-org-agenda--def-cmd "e" "Agenda：导出视图")
(eon-org-agenda--def-cmd "C" "配置 org-agenda-custom-commands" "custom-config")
(eon-org-agenda--def-cmd "!" "配置 org-stuck-projects" "stuck-config")

(defun eon-org-agenda--toggle-sticky ()
  "切换 sticky agenda views。"
  (interactive)
  (call-interactively #'org-toggle-sticky-agenda)
  (sit-for 1)
  (transient-setup 'eon-org-agenda))

(defun eon-org-agenda--restrict-buffer ()
  (interactive)
  (setq eon-org-agenda--restriction 'buffer)
  (eon-org-agenda--restriction-refresh))

(defun eon-org-agenda--restrict-subtree-or-region ()
  (interactive)
  (setq eon-org-agenda--restriction
        (if (org-region-active-p) 'region 'subtree))
  (eon-org-agenda--restriction-refresh))

(defun eon-org-agenda--restrict-clear ()
  (interactive)
  (setq eon-org-agenda--restriction nil)
  (when (fboundp 'org-agenda-remove-restriction-lock)
    (org-agenda-remove-restriction-lock 'noupdate))
  (eon-org-agenda--restriction-refresh))

;;;###autoload
(transient-define-prefix eon-org-agenda ()
  ...
  [["Agenda"
   ...
   ["查询"
    ...
    ("M" "TAGS（仅 TODO）" eon-org-agenda--M)
    ("s" "关键词搜索" eon-org-agenda--s)
    ("S" "搜索（仅 TODO）" eon-org-agenda--S)
    ("/" "multi-occur" eon-org-agenda--occur)
    ("?" "FLAGGED" eon-org-agenda--flagged)]
   ["其它"
    ("#" "stuck 项目" eon-org-agenda--stuck)
    ("e" "导出 agenda" eon-org-agenda--e)
    ("C" "配置 custom" eon-org-agenda--custom-config)
    ("!" "配置 stuck" eon-org-agenda--stuck-config)
    ("*" "sticky views" eon-org-agenda--toggle-sticky)]
   ["范围"
    ("1" "限 buffer" eon-org-agenda--restrict-buffer)
    ("0" "限 subtree/region" eon-org-agenda--restrict-subtree-or-region)
    (">" "取消限制" eon-org-agenda--restrict-clear)]]
  (interactive)
  (transient-setup 'eon-org-agenda))

;;;###autoload
(defun eon-org-agenda ()
  "打开 Org Agenda transient 菜单。"
  (interactive)
  (transient-setup 'eon-org-agenda))
...
(use-package org
  ...
  ;; 让 *bold* /italic/ +strike+ 等 emphasis 标记在中文字符旁也能识别
  ;; 默认 PRE/POST 只允许 ASCII 边界字符，导致 "中文+正文+中文" 不被识别
  ;; 在 PRE 与 POST 中加入 [:nonascii:] 即可
