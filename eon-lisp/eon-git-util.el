...
(defun eon-git-util-local-user-identity-configured-p ()
  "当前仓库是否已在本地配置 user.name 和 user.email。"
  (and (magit-get "--local" "user" "name")
       (magit-get "--local" "user" "email")))


(defun eon-git-util-set-local-user-identity (name email)
  "为当前仓库设置本地 user.name 和 user.email。"
  (magit-set name "--local" "user" "name")
  (magit-set email "--local" "user" "email"))
