...
(defun em-c-command ()
  ...
  (let ((keymap (make-sparse-keymap)))
    ...
    (if (and (buffer-file-name)
	     (string= (file-name-nondirectory (buffer-file-name)) ".eon.yaml"))
	(define-key keymap (kbd "c") #'eon-workspace-action-default))
...
(defun em-i ()
  (let ((realkey
	 (pcase major-mode
	   ...
	   ('ghostel-mode
	    (if (bound-and-true-p ghostel--input-mode)
                (if (memq ghostel--input-mode '(emacs copy))
                    #'eon-modalka-ghostel-to-char
                  #'em-disable)
              #'em-disable))
	   (_ #'em-disable))))
    realkey))
...
(defun eon-org-mark--bounds ()
  "返回需要标记的范围 (BEG . END)。
若 region 激活，使用 region 边界；
否则取光标处由非空白且非加号字符组成的最大连续段。
这样中文逗号、句号等标点不会被当作词边界。"
  (if (use-region-p)
      (cons (region-beginning) (region-end))
    (save-excursion
      (let* ((end (progn (skip-chars-forward "^ \t\r\n+") (point)))
	     (beg (progn (skip-chars-backward "^ \t\r\n+") (point))))
	(cons beg end)))))

(defun eon-org-mark-word-done-add ()
  (let* ((bounds (eon-org-mark--bounds))
	 (beg (car bounds))
	 (end (cdr bounds)))
    (save-excursion
      (goto-char end)
...
(defun eon-org-mark-word-done-toggle ()
  ...
  (let* ((bounds (eon-org-mark--bounds))
	 (beg (car bounds))
	 (end (cdr bounds))
	 (flag1 (save-excursion (goto-char beg) (eq (char-before) ?+)))
	 (flag2 (save-excursion (goto-char end) (eq (char-after) ?+))))
...
(defun em-n ()
  (let ((realkey
	 (pcase major-mode
	   ...
	   ('agent-shell-mode #'agent-shell-next-item)
	   (_ "C-n"))))
    realkey))
...
(defun em-o-command ()
  (interactive)
  (let ((keymap (make-sparse-keymap)))
    (define-key keymap (kbd "a") #'eon-org-agenda)
...
(defun em-p ()
  (let ((realkey
	 (pcase major-mode
	   ...
	   ('agent-shell-mode #'agent-shell-previous-item)
	   (_ "C-p"))))
    realkey))
